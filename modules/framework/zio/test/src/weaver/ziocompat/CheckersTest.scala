...
object CheckersTest extends SimpleZIOSuite with Checkers {
  ...
  test("IO form (1)") {
    forall { (a1: Int) =>
      zio.clock.sleep(100.millis).map(_ => expect(a1 * 2 == a1 + a1))
    }
  }

  test("IO form (2)") {
    forall { (a1: Int, a2: Int) =>
      zio.clock.sleep(1.second).map(_ => expect(a1 + a2 == a2 + a1))
    }
  }
