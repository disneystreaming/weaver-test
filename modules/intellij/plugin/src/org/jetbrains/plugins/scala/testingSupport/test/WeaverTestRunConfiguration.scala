...
class WeaverTestRunConfiguration(
  ...
  override def configurationProducer: AbstractTestConfigurationProducer[_] = WeaverTestConfigurationProducer

  override def testFramework: TestFramework = WeaverTestFramework

  override protected def validityChecker: SuiteValidityChecker = (clazz: PsiClass, suiteClass: PsiClass) => {
  ...
  override protected def runnerInfo: AbstractTestRunConfiguration.TestFrameworkRunnerInfo =
    TestFrameworkRunnerInfo("org.jetbrains.plugins.scala.testingSupport.test.WeaverTestRunner")
