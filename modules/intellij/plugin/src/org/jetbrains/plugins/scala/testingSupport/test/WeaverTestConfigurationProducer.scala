    ...
    extends AbstractTestConfigurationProducer[WeaverTestRunConfiguration]
{
  ...
  override def getConfigurationFactory: ConfigurationFactory = WeaverTestConfigurationType.configurationFactory

  override protected def suitePaths: Seq[String] =
    WeaverTestFramework.baseSuitePaths

  override protected def configurationName(contextInfo: AbstractTestConfigurationProducer.CreateFromContextInfo): String = contextInfo match {
    case CreateFromContextInfo.AllInPackage(_, packageName) => s"ScalaTests in ''$packageName''"
