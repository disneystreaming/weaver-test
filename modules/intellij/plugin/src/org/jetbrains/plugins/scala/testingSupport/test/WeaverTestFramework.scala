...
class WeaverTestFramework extends AbstractTestFramework with TestFrameworkSetupSupportBase {
  ...
  def frameworkSetupInfo(scalaVersion: Option[String]) =
    TestFrameworkSetupInfo(Seq(""""com.disneystreaming" %% "weaver-core" % "latest.integration" % "test""""), Seq())
